export default {
  async fetch(request) {
    ...
    const path = url.pathname.substring(1); // 获取去掉前导斜杠的路径
    ...
    if (path.startsWith("st/")) {
      const sizeStr = path.substring(3); // 截取 "st/" 后面的字符串
      const match = sizeStr.match(/^(\d+)([a-z]{0,2})$/i);
      ...
      if (match) {
        ...
        const bytes = size * (multipliers[unit] || 1);
    ...
    // 3. 所有其他情况（根目录、错误路径、格式不正确的 /st/xxx）直接返回 404
